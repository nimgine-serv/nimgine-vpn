#!/usr/bin/env python3
# File: /opt/nimgine/services/monitor/daemon.py
...
# --- CONFIGURATION ---
DB_PATH = "/opt/nimgine/core/database.db"
ONLINE_FILE = "/opt/nimgine/core/online_users.txt"
CHECK_INTERVAL = 30  

class nimgineMonitor:
    def __init__(self):
    ...
    def setup_iptables(self):
        try:
            subprocess.run("iptables -N nimgine-ACCT", shell=True, stderr=subprocess.DEVNULL)
            check_link = subprocess.run("iptables -C OUTPUT -j nimgine-ACCT", shell=True, stderr=subprocess.DEVNULL)
            if check_link.returncode != 0:
                subprocess.run("iptables -I OUTPUT -j nimgine-ACCT", shell=True, stderr=subprocess.DEVNULL)
