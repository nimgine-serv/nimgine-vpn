...
# --- Root Enforcement ---
if [ "${EUID}" -ne 0 ]; then
    echo -e "\033[0;31m[FATAL] You must be root to access the Nimgine™ Premium Script Dashboard.\033[0m"
...
fetch_server_geo() {
    local geo_file="/opt/nimgine/core/server_geo.env"
...
get_system_stats() {
    ...
    fetch_server_geo
    source /opt/nimgine/core/server_geo.env 2>/dev/null
...
execute_add_user() {
    while true; do
        ...
        /opt/nimgine/bin/nimgine user add "$USERNAME" "$PASSWORD" "$DAYS" "$MAX_LOGINS" "$BW_LIMIT" > /dev/null 2>&1
        API_STATUS=$?
...
execute_trial_user() {
    ...
    /opt/nimgine/bin/nimgine user trial "$USERNAME" "$PASSWORD" "$HOURS" "$MAX_LOGINS" "$BW_LIMIT" > /dev/null 2>&1
...
execute_renew_user() {
    ...
    /opt/nimgine/bin/nimgine user renew "$FINAL_USERNAME" "$MOD_DAYS" > /dev/null 2>&1
...
execute_del_user() {
    ...
    for target in "${TO_DELETE[@]}"; do
        /opt/nimgine/bin/nimgine user del "$target" > /dev/null 2>&1
...
execute_online_users() {
    ...
    local raw_data=$(/opt/nimgine/bin/nimgine sys online)
...
execute_user_details() {
    ...
    local PUB_KEY=$(cat /opt/nimgine/core/keys/dnstt.pub 2>/dev/null || echo "Missing Key")
...
print_user_receipt() {
    ...
    PUB_KEY=$(cat /opt/nimgine/core/keys/dnstt.pub 2>/dev/null || echo "Missing Key")
    ...
    while true; do
        ...
        case $opt in
            ...
            2) 
                ...
                if [[ -n "$new_ns" ]]; then
                    /opt/nimgine/bin/nimgine config ns "$new_ns"
                fi
                pause ;;
            3) 
                echo -e "\n${ORANGE}[*] Requesting Let's Encrypt Certificate. Services will temporarily pause...${NC}"
                /opt/nimgine/bin/nimgine cert renew "$PRIMARY_DOMAIN"
                pause ;;
            4) 
                clear
            ...
            5) 
                ...
                if [[ "$confirm_dnstt" =~ ^[Yy] ]]; then
                    /opt/nimgine/bin/nimgine dnstt renew
                fi
                pause ;;
            0) return ;;
            *) echo -e "${RED}Invalid option${NC}"; sleep 1 ;;
...
menu_services() {
    while true; do
        ...
        case $opt in
            1) /opt/nimgine/bin/nimgine service restart all; pause ;;
            2) /opt/nimgine/bin/nimgine service restart dropbear; pause ;;
            3) /opt/nimgine/bin/nimgine service restart nimgine-ws; pause ;;
            4) /opt/nimgine/bin/nimgine service restart stunnel4; pause ;;
            5) /opt/nimgine/bin/nimgine service restart nimgine-dnstt; pause ;;
            0) return ;;
            *) echo -e "${RED}Invalid option${NC}"; sleep 1 ;;
        esac
    done
    ...
    while true; do
        ...
        case $opt in
            ...
            2) 
                ...
                draw_line
                pause ;;
            3) tail -n 50 /opt/nimgine/logs/nimgine.log; pause ;;
            4) grep "Failed password" /var/log/auth.log | tail -n 20; pause ;;
            5) 
               if [ -x "/opt/nimgine/bin/btop" ]; then
                   /opt/nimgine/bin/btop
               else
                   htop || top
               fi
               pause ;;
    ...
    while true; do
        ...
        case $opt in
            1) 
                ...
                case $rb_opt in
                    1) /opt/nimgine/bin/nimgine sys autoreboot 6 ;;
                    2) /opt/nimgine/bin/nimgine sys autoreboot 12 ;;
                    3) /opt/nimgine/bin/nimgine sys autoreboot 24 ;;
                    0) /opt/nimgine/bin/nimgine sys autoreboot 0 ;;
            ...
            2) 
                ...
                /opt/nimgine/bin/nimgine sys banner
                
                pause ;;
            3) 
               clear
               echo -e "${CYAN}Initializing Server Speedtest...${NC}\n"
               if [ -x "/opt/nimgine/bin/speedtest" ]; then
                   /opt/nimgine/bin/speedtest --accept-license --accept-gdpr
               else
            ...
            5)
                ...
                rm -f /opt/nimgine/core/server_geo.env
                fetch_server_geo
            ...
            6)
                ...
                if [[ "$confirm_f2b" =~ ^[Yy] ]]; then
                    echo ""
                    /opt/nimgine/bin/nimgine sys fail2ban
                fi
                pause ;;
            0) return ;;
            *) echo -e "${RED}Invalid option${NC}"; sleep 1 ;;
    ...
    while true; do
        ...
        case $opt in
            1)
                ...
                /opt/nimgine/bin/nimgine sys backup
                pause
                ;;
            2)
                clear
                ...
                if [[ "$sel_idx" =~ ^[0-9]+$ ]] && [ "$sel_idx" -le "${#BACKUP_LIST[@]}" ] && [ "$sel_idx" -gt 0 ]; then
                    ...
                    if [ "$confirm_res" == "YES" ]; then
                        ...
                        /opt/nimgine/bin/nimgine sys restore "$target_file"
...
show_dashboard() {
    while true; do
        ...
        get_system_stats
        get_db_stats
        LIVE_CONNECTIONS=$(ps -eo user,comm | awk '$2 ~ /^(sshd|dropbear)$/ && $1 !~ /^(root|sshd|nobody|ubuntu|opc|admin|debian)$/' | wc -l)
        ...
        printf "  ${CYAN}WS-Proxy: %b   Stunnel : %b   Dropbear: %b${NC}\n" "$(check_service nimgine-ws)" "$(check_service stunnel4)" "$(check_service dropbear)"
        printf "  ${CYAN}Dante   : %b   UDP Cust: %b   DNSTT   : %b${NC}\n" "$(check_service danted)" "$(check_service nimgine-udp-custom)" "$(check_service nimgine-dnstt)"
        printf "  ${CYAN}Monitor : %b${NC}\n" "$(check_service nimgine-monitor)"
        echo -e ""
        ...
        case $opt in
            ...
            7) 
               clear
               echo -e "${CYAN}=== UPDATE NIMGINE™ PLATFORM ===${NC}"
               ...
               if [[ "$confirm_update" =~ ^[Yy] ]]; then
                   echo ""
                   /opt/nimgine/bin/nimgine sys update
                   pause
               fi
               ;;
            8) 
