...
# --- 3b. Dynamic Architecture & Binary Staging ---
echo -e "\n${CYAN}[*] Detecting Architecture & Fetching Binaries...${NC}"
ARCH=$(uname -m)
if [ "$ARCH" = "x86_64" ]; then
    BINARY_SUFFIX="amd64"
    ...
    echo -e "${RED}[FATAL] Unsupported architecture: $ARCH${NC}"
    exit 1
fi

echo -e "  -> Detected: ${ARCH} (Using suffix: ${BINARY_SUFFIX})"

# Fetch architecture-specific binaries directly into /opt/nimgine/bin
fetch_file "binaries/dnstt-server-${BINARY_SUFFIX}" "/opt/nimgine/bin/dnstt-server"
fetch_file "binaries/udp-custom-${BINARY_SUFFIX}" "/opt/nimgine/bin/udp-custom"

chmod +x /opt/nimgine/bin/dnstt-server
chmod +x /opt/nimgine/bin/udp-custom
