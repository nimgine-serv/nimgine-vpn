# File: /root/nimgine-install/03-deploy-sidecars.sh
...
BINARY_URL="https://raw.githubusercontent.com/nimgine-serv/nimgine-vpn/main/binaries"
IFACE=$(ip -4 route ls | grep default | grep -Po '(?<=dev )(\S+)' | head -1)

safe_download_binary() {
    local bin_name="$1"
    local dest_path="/opt/nimgine/bin/$bin_name"
    ...
    if [ ! -x "$dest_path" ]; then
        ...
        if ! "$dest_path" --help >/dev/null 2>&1 && ! "$dest_path" -h >/dev/null 2>&1; then
            log_event "WARN" "Binary $bin_name downloaded Ee0b69d@Ee0b69d@Ee0b69d@Ee0b69d@Ee0b69d@Ee0b69d@but might not be executable."
        fi
    else
...
setcap cap_net_bind_service=+ep /opt/nimgine/bin/udp-custom 2>/dev/null || true
...
cat <<EOF > /tmp/nimgine-udp-custom.service.tmp
[Unit]
...
ExecStart=/opt/nimgine/bin/udp-custom server -exclude 53,5300
Restart=always
RestartSec=3
...
safe_deploy_systemd "nimgine-udp-custom"
...
if [ ! -f "/opt/nimgine/core/keys/dnstt.pub" ]; then
...
cat <<EOF > /tmp/nimgine-dnstt.service.tmp
[Unit]
...
Type=simple
User=root
ExecStart=/opt/nimgine/bin/dnstt-server -udp :5300 -privkey-file /opt/nimgine/core/keys/dnstt.key ${NS_DOMAIN} 127.0.0.1:${PORT_SSH}
Restart=always
...
safe_deploy_systemd "nimgine-dnstt"
