# File: /root/nimgine-install/04-deploy-monitor.sh
...
chmod +x /opt/nimgine/services/monitor/daemon.py
...
if [ ! -x "/opt/nimgine/bin/speedtest" ]; then
    ...
    tar -xzf /tmp/speedtest.tgz -C /opt/nimgine/bin speedtest
    chmod +x /opt/nimgine/bin/speedtest
fi
...
if [ ! -x "/opt/nimgine/bin/btop" ]; then
    ...
    mv /tmp/btop/bin/btop /opt/nimgine/bin/btop
    chmod +x /opt/nimgine/bin/btop
    rm -rf /tmp/btop /tmp/btop.tbz
fi

# 3. Stage the Systemd file to temp
cat <<EOF > /tmp/nimgine-monitor.service.tmp

[Unit]
Description=nimgine Real-time Multi-Login Enforcer
After=network.target sqlite.target dropbear.service

[Service]
Type=simple
User=root
WorkingDirectory=/opt/nimgine/services/monitor
ExecStart=/usr/bin/python3 /opt/nimgine/services/monitor/daemon.py
Restart=always
RestartSec=5
ProtectSystem=full
ProtectHome=true
ReadWritePaths=/opt/nimgine/logs /opt/nimgine/core
...
safe_deploy_systemd "nimgine-monitor"
