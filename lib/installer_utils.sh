# File: /opt/nimgine/lib/installer_utils.sh
# Purpose: Idempotent helper functions for safe deployment.

# Ensure we have our logging function
source /opt/nimgine/lib/system.sh 2>/dev/null || echo "Warning: system.sh not found."
...
ensure_tls_cert() {
    local domain="$1"
    local cert_path="/opt/nimgine/core/keys/stunnel.pem"
    ...
    run_with_spinner "Generating TLS Certificate ($domain)..." openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
        -keyout /opt/nimgine/core/keys/private.key \
        -out /opt/nimgine/core/keys/fullchain.cer \
        -subj "/C=US/ST=NY/L=NY/O=nimgine/CN=$domain"
        
    cat /opt/nimgine/core/keys/fullchain.cer /opt/nimgine/core/keys/private.key > "$cert_path"
    chmod 600 "$cert_path"
