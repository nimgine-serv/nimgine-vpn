# File: /opt/nimgine/lib/services.sh
...
restart_service() {
    ...
    if [ "$service_name" == "all" ]; then
        log_event "INFO" "Restarting ALL nimgine services..."
        local services=(nimgine-ws nimgine-dnstt nimgine-monitor nimgine-udp-custom stunnel4 dropbear danted ssh sshd)
    ...
    case "$service_name" in
        dropbear|stunnel4|nimgine-ws|nimgine-dnstt|nimgine-monitor|danted|nimgine-udp-custom|ssh|sshd)
