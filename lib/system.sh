...
change_host_domain() {
    ...
    sed -i "s/PRIMARY_DOMAIN=.*/PRIMARY_DOMAIN=\"$new_domain\"/" /opt/nimgine/core/nimgine.conf
...
change_ns_domain() {
    ...
    sed -i "s/NS_DOMAIN=.*/NS_DOMAIN=\"$new_ns\"/" /opt/nimgine/core/nimgine.conf
    source /opt/nimgine/core/nimgine.conf
    
    # 2. Re-write the systemd service to use the new NS domain
    cat <<EOF > /etc/systemd/system/nimgine-dnstt.service
[Unit]
Description=nimgine DNSTT Server
After=network.target

[Service]
Type=simple
User=root
ExecStart=/opt/nimgine/bin/dnstt-server -udp :5300 -privkey-file /opt/nimgine/core/keys/dnstt.key ${new_ns} 127.0.0.1:${PORT_DROPBEAR}
Restart=always
...
renew_ssl_cert() {
    ...
    $ACME --installcert -d "$domain" \
        --fullchain-file /opt/nimgine/core/keys/fullchain.cer \
        --key-file /opt/nimgine/core/keys/private.key
        
    # 4. Bundle for Stunnel and Verify
    if [ -s /opt/nimgine/core/keys/fullchain.cer ]; then
        cat /opt/nimgine/core/keys/fullchain.cer /opt/nimgine/core/keys/private.key > /opt/nimgine/core/keys/stunnel.pem
        chmod 600 /opt/nimgine/core/keys/stunnel.pem
    ...
    # 5. Restore Services
    systemctl start nimgine-ws stunnel4
    log_event "INFO" "Data plane services restored."
}

generate_dnstt_key() {
    log_event "INFO" "Generating fresh DNSTT cryptographic keys..."
    cd /opt/nimgine/core/keys
    rm -f dnstt.key dnstt.pub
    
    /opt/nimgine/bin/dnstt-server -gen-key -privkey-file dnstt.key -pubkey-file dnstt.pub
    systemctl restart nimgine-dnstt
...
set_auto_reboot() {
    ...
    # Safely remove any existing nimgine reboot cron jobs
...
uninstall_script() {
    log_event "WARN" "Initiating complete uninstallation of Nimgine™ VPN Platform..."
    
    # 1. Stop and Disable all managed services
    local services=(nimgine-ws nimgine-dnstt nimgine-monitor nimgine-udp-custom stunnel4 dropbear danted)
    ...
    rm -f /etc/systemd/system/nimgine-*.service
    systemctl daemon-reload
    ...
    iptables -D OUTPUT -j nimgine-ACCT 2>/dev/null
    iptables -F nimgine-ACCT 2>/dev/null
    iptables -X nimgine-ACCT 2>/dev/null
    ...
    rm -f /usr/local/bin/nimgine /usr/local/sbin/menu
    ...
    # 6. Clean SSH config
    rm -f /etc/ssh/sshd_config.d/99-nimgine-banner.conf
...
safe_fetch() {
    local repo_url="https://raw.githubusercontent.com/nimgine-serv/nimgine-vpn/main"
    local file_path="$1"
    local target_path="$2"
    local tmp_dir="/tmp/nimgine_update"
...
update_script() {
    ...
    safe_fetch "lib/system.sh" "/opt/nimgine/lib/system.sh"
    safe_fetch "lib/users.sh" "/opt/nimgine/lib/users.sh"
    safe_fetch "lib/services.sh" "/opt/nimgine/lib/services.sh"
    safe_fetch "lib/db.sh" "/opt/nimgine/lib/db.sh"
    safe_fetch "lib/installer_utils.sh" "/opt/nimgine/lib/installer_utils.sh"
    
    # 2. Update APIs and Menus
    safe_fetch "bin/nimgine" "/opt/nimgine/bin/nimgine"
    safe_fetch "menus/main_menu.sh" "/opt/nimgine/menus/main_menu.sh"
    
    # 3. Update Python Services
    safe_fetch "services/monitor/daemon.py" "/opt/nimgine/services/monitor/daemon.py"
    safe_fetch "services/routing/async-ws-proxy.py" "/opt/nimgine/services/routing/ws-proxy.py"

    # 4. Database Migrations (Direct query to avoid infinite sourcing loop)
    echo -e "  \033[0;36m-> Running database migrations...\033[0m"
    local DB_PATH="/opt/nimgine/core/database.db"
    ...
    systemctl daemon-reload
    systemctl restart nimgine-ws nimgine-monitor >/dev/null 2>&1
...
create_backup() {
    local backup_dir="/opt/nimgine/backups"
    mkdir -p "$backup_dir"
    local timestamp=$(date +"%Y%m%d_%H%M%S")
    local backup_file="$backup_dir/nimgine_backup_$timestamp.tar.gz"
    ...
    tar -czf "$backup_file" core/database.db core/nimgine.conf core/keys/ core/vpn_passwd.bak core/vpn_shadow.bak >/dev/null 2>&1
...
restore_backup() {
    ...
    tar -xzf "$temp_archive" -C /opt/nimgine >/dev/null 2>&1
    rm -f "$temp_archive"

    # Restore VPN Linux users safely
    if [ -f /opt/nimgine/core/vpn_passwd.bak ]; then
        ...
        done < /opt/nimgine/core/vpn_passwd.bak
        rm -f /opt/nimgine/core/vpn_passwd.bak
    fi

    if [ -f /opt/nimgine/core/vpn_shadow.bak ]; then
        while IFS= read -r line; do
            local uname=$(echo "$line" | cut -d: -f1)
            sed -i "/^$uname:/d" /etc/shadow
            echo "$line" >> /etc/shadow
        done < /opt/nimgine/core/vpn_shadow.bak
        rm -f /opt/nimgine/core/vpn_shadow.bak
    fi

    chmod 600 /opt/nimgine/core/keys/* 2>/dev/null || true
    systemctl restart nimgine-ws nimgine-dnstt stunnel4 dropbear >/dev/null 2>&1
